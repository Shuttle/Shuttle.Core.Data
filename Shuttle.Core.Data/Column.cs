...
public class Column : IColumn
{
    ...
    {
        return Guard.AgainstNull(row)[Name];
    }

    public object RawValue(IDataRecord record)
    {
        return Guard.AgainstNull(record)[Name];
    }

    public bool IsNull(dynamic instance)
    {
        return Guard.AgainstNull(instance)[Name] == DBNull.Value;
    }

    public bool IsNull(DataRow row)
    {
        return Guard.AgainstNull(row).IsNull(Name);
    }

    public bool IsNull(IDataRecord record)
    {
        return Guard.AgainstNull(record).IsDBNull(Ordinal(record));
    }
    ...
    public Column Rename(string name)
    {
        ...
        return Size.HasValue
            ? new(name, Type, DbType, Size.Value)
            : Precision.HasValue
                ? new(name, Type, DbType, Precision.Value, Scale ?? 0)
...
public class Column<T> : Column
{
    ...
    public new Column<T> Rename(string name)
    {
        ...
        return Size.HasValue
            ? new(name, DbType, Size.Value)
            : Precision.HasValue
                ? new(name, DbType, Precision.Value, Scale ?? 0)
